...
- Database (RDS PostgreSQL or BYODB ClickHouse) — *BYODB = Bring Your Own Database: you provide an existing database endpoint, evm-cloud connects to it*
- Secrets Manager for credentials
- IAM roles with least-privilege policies

> **Cost note:** EKS deployments with private subnets include a NAT Gateway (~$32-45/mo). This is in addition to the EKS control plane fee (~$73/mo). See [Cost Estimates](./cost-estimates.mdx) for full breakdowns.
...
**Phase 1: Infrastructure** — `evm-cloud deploy` (or `terraform apply`)
...
**Phase 2: Workloads** — `evm-cloud deploy --only app`
