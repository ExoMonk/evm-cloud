...
**Phase 2: Workloads** — deployer script

Uses the kubeconfig from Phase 1 to deploy Helm charts onto the cluster.

```bash
cd deployers/k3s
./deploy.sh ../../examples/k3s_rds
```

The deployer script reads `workload_handoff` from the Terraform state, extracts the kubeconfig, and runs `helm upgrade --install` for each service.
