...
```bash
# Verify (IP shown in post-deploy summary)
ssh -i ~/.ssh/evm-cloud ec2-user@<public-ip> 'sudo docker compose -f /opt/evm-cloud/docker-compose.yml ps'
```
...
| `ec2_rpc_proxy_mem_limit` | string | `1g` | eRPC container memory limit |
| `ec2_indexer_mem_limit` | string | `2g` | rindexer container memory limit |
