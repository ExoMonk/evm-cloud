...
| **Webhooks** | Real-time event streaming to your API via rindexer's built-in webhook/stream support. Add a `streams` section to your `rindexer.yaml` to push decoded events to any HTTP endpoint in sub-5ms. |
...
output "workload_handoff" {
  value = {
    ...
    runtime = {
      ec2        = { public_ip, host_address, ssh_user }
      eks        = { cluster_name, cluster_endpoint, oidc_provider_arn }
      k3s        = { host_ip, kubeconfig_base64 }
      bare_metal = { host_address }
    }
    services = { rpc_proxy = { ... }, indexer = { ... }, monitoring = { ... } }
    data     = { backend, clickhouse = { ... }, postgres = { ... } }
    secrets  = { mode, ... }
    ingress  = { mode, erpc_hostname, ... }
  }
}
```
...
| **core/k8s/addons/monitoring** | `modules/core/k8s/addons/monitoring/` | kube-prometheus-stack + Loki + Promtail addon. Deploys Prometheus, Grafana, Alertmanager, dashboards, and alert rules. |
...
root module (main.tf)
  ...
  │
  ├── modules/core/k8s/addons/           # K8s addon modules
...
- **[Examples](./examples/index.mdx)** -- 9 runnable deployment patterns
