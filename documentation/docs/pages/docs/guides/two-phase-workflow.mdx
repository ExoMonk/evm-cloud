...
```bash
# Check pods are running (namespace = your project name)
evm-cloud kubectl get pods -n <project-name>
...
The workload namespace is derived from your `project_name` (lowercased, sanitized to DNS-1123). For example, project `my-indexer` deploys to namespace `my-indexer`.
