...
### Use evm-cloud CLI (simplest)

The CLI handles kubeconfig extraction and deploy orchestration automatically:

```bash
evm-cloud deploy              # deploy workloads
evm-cloud kubectl get pods    # kubectl with auto-resolved kubeconfig
evm-cloud destroy --yes       # teardown workloads + infrastructure
```
...
:::warning
The `evm-cloud deploy` CLI command does not currently support EKS workload deployment. Use `evm-cloud apply` for infrastructure provisioning, then run the deployer script below manually.
:::
